...
const LineGraph = () => {
    // the store to store all the data in the state
    ...
    //b ecause chart needs to have x and y coordinates, where x is the date, and the y is the cases number, in this way
    ...
    const buildChartData = (data, casesType='cases') => {
        ...
        data[casesType].cases.forEach(date => {
            if(lastDataPoint) {
                const newDataPoint = {
                    x: date,
                    // y becayse we are going to show it for the cases, and it goes to the data object, to the cases, to the cases date, and
